            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="bg-white">
                    <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                        ...
                        <form method="POST" action="{{ route('cities.update', [$city]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data">
                                    ...
                                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                                        <select id="district_id" name="district_id" class="max-w-lg block focus:ring-indigo-500 focus:border-indigo-500 w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                            @foreach ($districts as $district)
                                                <option {{ $city->district_id == $district->id ? 'selected' : '' }}  value="{{ $district->id }}">{{ $district->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                    ...
                                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                                        <select id="status" name="status" class="max-w-lg block focus:ring-indigo-500 focus:border-indigo-500 w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                            <option {{ $city->status == '1' ? 'selected' : '' }}  value="1">Enabled</option>
                                            <option {{ $city->status == '0' ? 'selected' : '' }} value="0">Disabled</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
