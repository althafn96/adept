    ...
    <x-slot name="header">
        <div class="-ml-4 -mt-4 flex justify-between items-center flex-wrap sm:flex-nowrap">
            ...
            <div class="ml-4 mt-4 flex-shrink-0">
                <a href="{{ route('districts.index') }}" type="button"
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="bg-white">
                    <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                        ...
                        <form method="POST" action="{{ route('districts.update', [$district]) }}" class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data">
                                ...
                                <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                    ...
                                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                                        <select id="status" name="status" class="max-w-lg block focus:ring-adeptred-1 focus:border-adeptred-1 w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                            <option {{ $district->status == '1' ? 'selected' : '' }}  value="1">Enabled</option>
                                            <option {{ $district->status == '0' ? 'selected' : '' }} value="0">Disabled</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
